...
def draw_capacity_diagram(s,d_range:Tuple[float,float],L_range:Tuple[float,float]):
    ...
    lines=[200,300,400,500,600,800,1000,1250,1500,2000,3000,5000,7500,10000]
    fig, ax =  plt.subplots()
    ax.grid(True)
...
def draw_utilization_diagram(N,s,d_range:Tuple[float,float],L_range:Tuple[float,float]):
    ...
    ax.clabel(ax.contour(X,Y, Za, np.round(np.linspace(0,1,11),1), colors='red'), inline=True, fontsize=8)
    ax.yaxis.set_inverted(True)
    ax.xaxis.tick_top()
    return fig
